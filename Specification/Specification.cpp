...
int main(int argc, char* argv[]) {
	...
	match = uc_alloc(Row, Col);
	outimg = uc_alloc(Row, Col);
	read_ucmartrix(Row, Col, img, argv[1]);
	read_ucmartrix(Row, Col, match, argv[2]);



	get_cdf(img, cdfOfHisto_origin, Row, Col); // 입력 이미지의 cdf 얻기
	for (i = 0; i < 256; i++)
		printf("origin cdf histomathch [%d] : %d\n", i, cdfOfHisto_origin[i]); // 얻어온 cdf 출력
	get_cdf(match, cdfOfHisto_match, Row, Col);// 지정 이미지의 cdf 얻기
	for (i = 0; i < 256; i++)
		printf("match cdf histomathch [%d] : %d\n", i, cdfOfHisto_origin[i]);



	get_Match(img,outimg, Row, Col, cdfOfHisto_match);
